...
rule unicycler:
    ...
    shell:
        """
        unicycler -1 {input[1]} -2 {input[2]} -s {input[3]} -l {input[0]} -o {params.outdir} --keep {params.keep} --mode {params.mode} --linear_seqs {params.linear} --min_fasta_length {params.min} -t {threads} --verbosity 3 &>> {log}
        """
